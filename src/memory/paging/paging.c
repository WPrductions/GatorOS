...
uint32_t* paging_4gb_chnuk_get_directory(struct paging_4gb_chunk* chunk)
{
...
int paging_get_indeces(void* virtual_address, uint32_t* directory_index_out, uint32_t* table_index_out)
{
    int res = 0;
...
int paging_set(uint32_t* dir, void* virt, uint32_t val)
{
    ...
    int res = paging_get_indeces(virt, &dir_index, &table_index);
    if(res < 0)
        return res;
